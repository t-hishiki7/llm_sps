...
class Agent:
    ...
    def calc(self):
        ...
        system_message = f"""
        You are an AI agent participating in the Social Particle Swarm (SPS) model experiment. 
        Your role is to make decisions that maximize your total payoff while interacting with other agents in a simulated environment.
        """
       ...
       - Payoff structure: Based on the Prisoner's Dilemma game.
         * Both Cooperate (CC): Both gain moderate payoff ({PR})
         * Both Defect (DD): Both receive small loss ({PP})
         * One Cooperates, One Defects (CD):
           - Cooperator: Severe loss ({PS}) (sucker's payoff)
           - Defector: Large payoff ({PT}) (temptation payoff)
       - Important: The payoff gained in one step is divided by the distance between particles. This means interactions with closer particles have a greater impact.
        ...
        {"Current Personality Traits(each trait is represented on a scale from 0 to 1, where 0 indicates a low level of the trait, 0.5 is average, and 1 indicates a high level):" + json.dumps(vars(self.personality), indent=2) if USE_PERSONALITY else ""}
